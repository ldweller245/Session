import QtQuick 2.0
import Felgo 3.0
import "../Components"

Page {
    id: canvas
    Rectangle {
        id: frameBase; width: parent.width; height: parent.height; anchors.centerIn: parent; color: canvasBG === undefined ? "#cccccc" :  canvasBG
        Row {
            anchors.fill: parent
            Column {
                height: parent.height
                width: parent.width/3
                Rectangle {
                    height: parent.height/3; width: parent.width; color: "white"; scale: rect1Scale; border.width: currentRect === "rect1" ? dp(5) : 0; border.color: "steelblue";
                    Rectangle{z: 2; anchors.fill: parent; color: "steelblue"; opacity: currentRect === "rect1" ? 0.5 : 0}
                    AppImage {z: 1;id: rect1Img; source: rect1Imgsource; anchors.fill: parent; autoTransform: true; smooth: true; fillMode: Image.PreserveAspectCrop}
                    MouseArea {
                        anchors.fill: parent
                        onClicked: {
                            if(currentRect === "rect1"){currentRect = "none"}
                            else {currentRect = "rect1"; slider.value = parent.scale}
                        }
                    }
                    IconButton {
                        z: 3; icon: IconType.edit; anchors.bottom: parent.bottom; anchors.right: parent.right
                        onClicked: {imagePickerModal.open(); currentRect = "rect1"}
                    }
                }
                Rectangle {
                    height: parent.height/3; width: parent.width; color: "white"; scale: rect4Scale; border.width: currentRect === "rect4" ? dp(5) : 0; border.color: "steelblue";
                    Rectangle{z: 2; anchors.fill: parent; color: "steelblue"; opacity: currentRect === "rect4" ? 0.5 : 0}
                    AppImage {z: 1; id: rect4Img; source: rect4Imgsource;anchors.fill: parent; autoTransform: true; smooth: true; fillMode: Image.PreserveAspectCrop}
                    MouseArea {
                    anchors.fill: parent
                    onClicked: {
                        if(currentRect === "rect4"){currentRect = "none"}
                        else {currentRect = "rect4"; slider.value = parent.scale}
                        }
                    }
                    IconButton {
                        z: 3; icon: IconType.edit; anchors.bottom: parent.bottom; anchors.right: parent.right
                        onClicked: {imagePickerModal.open(); currentRect = "rect4"}
                    }
                }
                Rectangle {
                    height: parent.height/3; width: parent.width; color: "white"; scale: rect7Scale; border.width: currentRect === "rect7" ? dp(5) : 0; border.color: "steelblue";
                    Rectangle{z: 2; anchors.fill: parent; color: "steelblue"; opacity: currentRect === "rect7" ? 0.5 : 0}
                    AppImage {z: 1;id: rect7Img; source: rect7Imgsource; anchors.fill: parent; autoTransform: true; smooth: true; fillMode: Image.PreserveAspectCrop}
                    MouseArea {
                        anchors.fill: parent
                        onClicked: {
                            if(currentRect === "rect7"){currentRect = "none"}
                            else {currentRect = "rect7"; slider.value = parent.scale}
                        }
                    }
                    IconButton {
                        z: 3; icon: IconType.edit; anchors.bottom: parent.bottom; anchors.right: parent.right
                        onClicked: {imagePickerModal.open(); currentRect = "rect7"}
                    }
                }
            }
            Column {
                height: parent.height
                width: parent.width/3
                Rectangle {
                    height: parent.height/3; width: parent.width; color: "white"; scale: rect2Scale; border.width: currentRect === "rect2" ? dp(5) : 0; border.color: "steelblue";
                    Rectangle{z: 2; anchors.fill: parent; color: "steelblue"; opacity: currentRect === "rect2" ? 0.5 : 0}
                    AppImage {z: 1; id: rect2Img; source: rect2Imgsource; anchors.fill: parent; autoTransform: true; smooth: true; fillMode: Image.PreserveAspectCrop}
                    MouseArea {
                        anchors.fill: parent
                        onClicked: {
                            if(currentRect === "rect2"){currentRect = "none"}
                            else {currentRect = "rect2"; slider.value = parent.scale}
                        }
                    }
                    IconButton {
                        z: 3; icon: IconType.edit; anchors.bottom: parent.bottom; anchors.right: parent.right
                        onClicked: {imagePickerModal.open(); currentRect = "rect2"}
                    }
                }
                Rectangle {
                    height: parent.height/3; width: parent.width; color: "white"; scale: rect5Scale; border.width: currentRect === "rect5" ? dp(5) : 0; border.color: "steelblue";
                    Rectangle{z: 2; anchors.fill: parent; color: "steelblue"; opacity: currentRect === "rect5" ? 0.5 : 0}
                    AppImage {z: 1; id: rect5Img; source: rect5Imgsource;anchors.fill: parent; autoTransform: true; smooth: true; fillMode: Image.PreserveAspectCrop}
                    MouseArea {
                    anchors.fill: parent
                    onClicked: {
                        if(currentRect === "rect5"){currentRect = "none"}
                        else {currentRect = "rect5"; slider.value = parent.scale}
                        }
                    }
                    IconButton {
                        z: 3; icon: IconType.edit; anchors.bottom: parent.bottom; anchors.right: parent.right
                        onClicked: {imagePickerModal.open(); currentRect = "rect5"}
                    }
                }
                Rectangle {
                    height: parent.height/3; width: parent.width; color: "white"; scale: rect8Scale; border.width: currentRect === "rect8" ? dp(5) : 0; border.color: "steelblue";
                    Rectangle{z: 2; anchors.fill: parent; color: "steelblue"; opacity: currentRect === "rect8" ? 0.5 : 0}
                    AppImage {z: 1;id: rect8Img; source: rect8Imgsource; anchors.fill: parent; autoTransform: true; smooth: true; fillMode: Image.PreserveAspectCrop}
                    MouseArea {
                        anchors.fill: parent
                        onClicked: {
                            if(currentRect === "rect8"){currentRect = "none"}
                            else {currentRect = "rect8"; slider.value = parent.scale}
                        }
                    }
                    IconButton {
                        z: 3; icon: IconType.edit; anchors.bottom: parent.bottom; anchors.right: parent.right
                        onClicked: {imagePickerModal.open(); currentRect = "rect8"}
                    }
                }
            }
            Column {
                height: parent.height
                width: parent.width/3
                Rectangle {
                    height: parent.height/3; width: parent.width; color: "white"; scale: rect3Scale; border.width: currentRect === "rect3" ? dp(5) : 0; border.color: "steelblue";
                    Rectangle{z: 2; anchors.fill: parent; color: "steelblue"; opacity: currentRect === "rect3" ? 0.5 : 0}
                    AppImage {z: 1; id: rect3Img; source: rect3Imgsource; anchors.fill: parent; autoTransform: true; smooth: true; fillMode: Image.PreserveAspectCrop}
                    MouseArea {
                    anchors.fill: parent
                    onClicked: {
                        if(currentRect === "rect3"){currentRect = "none"}
                        else {currentRect = "rect3"; slider.value = parent.scale}
                        }
                    }
                    IconButton {
                        z: 3; icon: IconType.edit; anchors.bottom: parent.bottom; anchors.right: parent.right
                        onClicked: {imagePickerModal.open(); currentRect = "rect3"}
                    }
                }
                Rectangle {
                    height: parent.height/3; width: parent.width; color: "white"; scale: rect6Scale; border.width: currentRect === "rect6" ? dp(5) : 0; border.color: "steelblue";
                    Rectangle{z: 2; anchors.fill: parent; color: "steelblue"; opacity: currentRect === "rect6" ? 0.5 : 0}
                    AppImage {z: 1; id: rect6Img; source: rect6Imgsource;anchors.fill: parent; autoTransform: true; smooth: true; fillMode: Image.PreserveAspectCrop}
                    MouseArea {
                    anchors.fill: parent
                    onClicked: {
                        if(currentRect === "rect6"){currentRect = "none"}
                        else {currentRect = "rect6"; slider.value = parent.scale}
                        }
                    }
                    IconButton {
                        z: 3; icon: IconType.edit; anchors.bottom: parent.bottom; anchors.right: parent.right
                        onClicked: {imagePickerModal.open(); currentRect = "rect6"}
                    }
                }
                Rectangle {
                    height: parent.height/3; width: parent.width; color: "white"; scale: rect9Scale; border.width: currentRect === "rect9" ? dp(5) : 0; border.color: "steelblue";
                    Rectangle{z: 2; anchors.fill: parent; color: "steelblue"; opacity: currentRect === "rect9" ? 0.5 : 0}
                    AppImage {z: 1;id: rect9Img; source: rect9Imgsource; anchors.fill: parent; autoTransform: true; smooth: true; fillMode: Image.PreserveAspectCrop}
                    MouseArea {
                        anchors.fill: parent
                        onClicked: {
                            if(currentRect === "rect9"){currentRect = "none"}
                            else {currentRect = "rect9"; slider.value = parent.scale}
                        }
                    }
                    IconButton {
                        z: 3; icon: IconType.edit; anchors.bottom: parent.bottom; anchors.right: parent.right
                        onClicked: {imagePickerModal.open(); currentRect = "rect9"}
                    }
                }
            }
        }
    }
}
